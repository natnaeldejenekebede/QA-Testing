<template>
  ...
  >
    ...
    >
      <div class="p-10">
        <NextIconSection text="Password recovery" />
        <InfoBaner
          img-src="https://upload.wikimedia.org/wikipedia/commons/b/b9/2023_Facebook_icon.svg"
          title="Add New Password"
          content-text="Your account uses
FaceBook sign-in add a new 
password"
          class="border border-blue-700 p-5 m-5"
        />
        <Text text="Enter Recovery Email" class="text-white text-center" />
        <Text
          text="A recovery mail will be sent to this address. 
         "
          class="text-white break-words"
        />
        <TransitionState class="m-15" />

        <ActionButton
          @pointerup="navigateToResetPassword"
          text="Reset Password"
        />
      </div>
    </div>
...
import NextIconSection from '~/components/molecules/NextIconSection.vue';import { store } from '~/store/store';
